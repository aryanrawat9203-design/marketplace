...
export default function GuidesIndexPage() {
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      ...
      <h1 className="mt-2.5 text-3xl font-bold tracking-tight text-zinc-50">Guides</h1>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {guides.map((g) => (
          <Link key={g.slug} href={`/guides/${g.slug}`} className="card card-hover group flex flex-col p-5">
            <h2 className="font-sans font-semibold text-zinc-100 group-hover:text-white">{g.title}</h2>
            <p className="mt-2 flex-1 text-sm leading-relaxed text-zinc-400">{g.description}</p>
